alter table Rating drop foreign key FK917A9DBDF8646883;
alter table Rating drop foreign key FK917A9DBDC96A3395;
alter table RatingCategory drop foreign key FKECF0B1DB8E895F60;
...
alter table RatingObject drop foreign key FKDAB02B5C8E895F60;
...
create table Rating (RATING_ID integer not null auto_increment unique, RATING_ANONYM bit not null, RATING_FEEDBACK varchar(255), RATING_OBJECT_ID integer not null, RATING_USER integer, primary key (RATING_ID)) ENGINE=InnoDB;
create table RatingCategory (RATING_CATEGORY_ID integer not null auto_increment unique, RATING_CATEGORY_DESCRIPTION varchar(255), RATING_CATEGORY_NAME varchar(255) not null unique, RATING_CATEGORY_WEIGHT float not null, RATING_SYTEM integer not null, primary key (RATING_CATEGORY_ID)) ENGINE=InnoDB;
...
create table RatingObject (RATING_OBJECT_ID integer not null auto_increment unique, RATING_OBJECT_OBJECT_ID varchar(255) not null unique, RATING_SYTEM integer not null, primary key (RATING_OBJECT_ID)) ENGINE=InnoDB;
create table RatingSystem (RATING_SYSTEM_ID integer not null auto_increment unique, RATING_SYSTEM_DESCRIPTION varchar(255), RATING_SYSTEM_NAME varchar(255) not null unique, primary key (RATING_SYSTEM_ID)) ENGINE=InnoDB;
...
alter table Rating add index FK917A9DBDF8646883 (RATING_USER), add constraint FK917A9DBDF8646883 foreign key (RATING_USER) references Localuser (LOCALUSER_ID);
...
alter table RatingCategory add index FKECF0B1DB8E895F60 (RATING_SYTEM), add constraint FKECF0B1DB8E895F60 foreign key (RATING_SYTEM) references RatingSystem (RATING_SYSTEM_ID);
...
alter table RatingObject add index FKDAB02B5C8E895F60 (RATING_SYTEM), add constraint FKDAB02B5C8E895F60 foreign key (RATING_SYTEM) references RatingSystem (RATING_SYSTEM_ID);
